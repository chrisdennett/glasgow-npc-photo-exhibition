import React from "react";

export default function MuseumMap() {
  const opacity = 0.5;

  return (
    <svg
      viewBox="0 0 54.635 170.598"
      width={140}
      style={{ margin: "0 2vw 42px 2vw" }}
    >
      <g transform="translate(-20.29 -17.373)">
        {/* YOU ARE HERE KEY */}
        <g>
          <g
            id="youAreHereIcon"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeWidth="1.043"
            color="#000"
            paintOrder="markers fill stroke"
            transform="matrix(1.01485 0 0 1.01462 12.6 -61.995)"
          >
            <circle
              cx="14.099"
              cy="83.128"
              r="3.338"
              fill="#fff"
              stroke="rgb(168, 194, 94)"
              strokeOpacity="1"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <circle
              cx="14.099"
              cy="83.128"
              r="1.719"
              fill="rgb(168, 194, 94)"
              stroke="none"
              strokeOpacity="0.469"
              overflow="visible"
              stopColor="#000"
            ></circle>
          </g>
          <text
            x="67.876"
            y="23.743"
            fill="none"
            stroke="#000"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="0.469"
            strokeWidth="1.058"
            color="#000"
            fontFamily="Alegreya Sans SC"
            fontSize="11.289"
            fontWeight="700"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
            style={{
              lineHeight: "125%",
              InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
              WebkitTextAlign: "end",
              textAlign: "end",
            }}
            textAnchor="end"
            writingMode="lr-tb"
            xmlSpace="preserve"
          >
            <tspan
              x="67.876"
              y="23.743"
              fill="#000"
              fillOpacity="0.431"
              stroke="none"
              strokeWidth="1.058"
              fontSize="5.644"
            >
              = You are here
            </tspan>
          </text>
        </g>

        <g style={{ opacity }}>
          <path
            fill="#7c7c7c"
            fillOpacity="1"
            stroke="none"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M20.82 44.718v-15.08h53.51v15.08H34.72z"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            fillOpacity="1"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M30.372 45.096h-9.553V29.165h22.744m10.559 0h20.274v15.931H39.104"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>

          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M47.682 71.592v6.267H20.86V45.03m26.822 0v18.823"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M47.66 77.859h12.085m8.844 0h5.782V45.03"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-69.82 -59.798)"
          >
            <circle
              cx="103.726"
              cy="121.378"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="101.63"
              y="123.659"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="101.63"
                y="123.659"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                A
              </tspan>
            </text>
          </g>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-45.61 -76.713)"
          >
            <circle
              cx="107.298"
              cy="137.82"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="105.201"
              y="140.103"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="105.201"
                y="140.103"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                B
              </tspan>
            </text>
          </g>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-66.645 -75.144)"
          >
            <circle
              cx="113.669"
              cy="163.155"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="111.571"
              y="165.441"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="111.571"
                y="165.441"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                C
              </tspan>
            </text>
          </g>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-81.462 -75.144)"
          >
            <circle
              cx="128.485"
              cy="183.709"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="126.386"
              y="185.998"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="126.386"
                y="185.998"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                D
              </tspan>
            </text>
          </g>
          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M30.401 98.266h-9.54V77.897m53.51 0v20.369h-35.25"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M64.83 118.563h9.541v-20.37m-53.51 0v20.37H56.11"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M47.682 145.146v6.267H20.86v-32.828m26.822 0v18.824"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M47.66 151.413h9.968m6.917 0h9.826v-32.828"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-69.82 -59.798)"
          >
            <circle
              cx="103.726"
              cy="195.461"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="101.63"
              y="197.751"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="101.63"
                y="197.751"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                E
              </tspan>
            </text>
          </g>
          <g
            fillOpacity="1"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-69.82 -59.798)"
          >
            <circle
              cx="131.507"
              cy="194.988"
              r="5.717"
              fill="#7c7c7c"
              stroke="none"
              strokeOpacity="1"
              strokeWidth="0.529"
              overflow="visible"
              stopColor="#000"
            ></circle>
            <text
              x="129.408"
              y="197.279"
              fill="#fff"
              stroke="#000"
              strokeOpacity="0.469"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="14.085"
              fontWeight="700"
              overflow="visible"
              stopColor="#000"
              style={{
                lineHeight: "125%",
                InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                WebkitTextAlign: "end",
                textAlign: "end",
              }}
              textAnchor="end"
              transform="scale(1.00012 .99988)"
              writingMode="lr-tb"
              xmlSpace="preserve"
            >
              <tspan
                x="129.408"
                y="197.279"
                fill="#fff"
                fillOpacity="1"
                stroke="none"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="7.042"
                fontStretch="normal"
                fontStyle="normal"
                fontVariant="normal"
                fontWeight="700"
                style={{
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "start",
                  textAlign: "start",
                }}
                textAnchor="start"
              >
                F
              </tspan>
            </text>
          </g>
          <text
            x="62.782"
            y="39.387"
            fill="#fff"
            fillOpacity="0.972"
            stroke="#000"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="0.469"
            strokeWidth="1.058"
            color="#000"
            fontFamily="Alegreya Sans SC"
            fontSize="7.042"
            fontWeight="700"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
            style={{
              lineHeight: "125%",
              InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
              WebkitTextAlign: "end",
              textAlign: "end",
            }}
            textAnchor="end"
            transform="scale(1.00012 .99988)"
            writingMode="lr-tb"
            xmlSpace="preserve"
          >
            <tspan
              x="63.841"
              y="39.387"
              fill="#fff"
              fillOpacity="0.972"
              stroke="none"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="7.042"
              fontStretch="normal"
              fontStyle="normal"
              fontVariant="normal"
              fontWeight="700"
              style={{}}
            >
              Entrance
            </tspan>
          </text>
          <path
            fill="#7c7c7c"
            fillOpacity="1"
            stroke="none"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M20.82 187.064v-15.08h53.51v15.08H34.72z"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <path
            fill="none"
            fillOpacity="1"
            stroke="#7c7c7c"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="1"
            strokeWidth="1.058"
            d="M74.33 171.51v15.932H20.82V171.51"
            color="#000"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
          ></path>
          <text
            x="68.742"
            y="181.75"
            fill="#fff"
            fillOpacity="0.972"
            stroke="#000"
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            strokeOpacity="0.469"
            strokeWidth="1.058"
            color="#000"
            fontFamily="Alegreya Sans SC"
            fontSize="7.042"
            fontWeight="700"
            overflow="visible"
            paintOrder="markers fill stroke"
            stopColor="#000"
            style={{
              lineHeight: "125%",
              InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
              WebkitTextAlign: "end",
              textAlign: "end",
            }}
            textAnchor="end"
            transform="scale(1.00012 .99988)"
            writingMode="lr-tb"
            xmlSpace="preserve"
          >
            <tspan
              x="68.742"
              y="181.75"
              fill="#fff"
              fillOpacity="0.972"
              stroke="none"
              strokeWidth="1.058"
              fontFamily="Alegreya Sans SC"
              fontSize="7.042"
              fontStretch="normal"
              fontStyle="normal"
              fontVariant="normal"
              fontWeight="700"
              style={{}}
            >
              INFORMATION
            </tspan>
          </text>

          <g
            strokeDasharray="none"
            strokeLinejoin="round"
            strokeMiterlimit="4"
            color="#000"
            paintOrder="markers fill stroke"
            transform="translate(-.06 .326)"
          >
            <g fillOpacity="1" transform="translate(-66.645 -2.119)">
              <circle
                cx="113.669"
                cy="163.155"
                r="5.717"
                fill="#7c7c7c"
                stroke="none"
                strokeOpacity="1"
                strokeWidth="0.529"
                overflow="visible"
                stopColor="#000"
              ></circle>
              <text
                x="111.571"
                y="165.441"
                fill="#fff"
                stroke="#000"
                strokeOpacity="0.469"
                strokeWidth="1.058"
                fontFamily="Alegreya Sans SC"
                fontSize="14.085"
                fontWeight="700"
                overflow="visible"
                stopColor="#000"
                style={{
                  lineHeight: "125%",
                  InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                  WebkitTextAlign: "end",
                  textAlign: "end",
                }}
                textAnchor="end"
                transform="scale(1.00012 .99988)"
                writingMode="lr-tb"
                xmlSpace="preserve"
              >
                <tspan
                  x="111.571"
                  y="165.441"
                  fill="#fff"
                  fillOpacity="1"
                  stroke="none"
                  strokeWidth="1.058"
                  fontFamily="Alegreya Sans SC"
                  fontSize="7.042"
                  fontStretch="normal"
                  fontStyle="normal"
                  fontVariant="normal"
                  fontWeight="700"
                  style={{
                    InkscapeFontSpecification: "'Alegreya Sans SC Bold'",
                    WebkitTextAlign: "start",
                    textAlign: "start",
                  }}
                  textAnchor="start"
                >
                  G
                </tspan>
              </text>
            </g>
            <path
              fill="none"
              stroke="#7c7c7c"
              strokeOpacity="1"
              strokeWidth="1.058"
              d="M30.46 171.29h-9.54v-20.368m53.51 0v20.369H39.182"
              overflow="visible"
              stopColor="#000"
            ></path>
          </g>
        </g>
        <use
          width="100%"
          height="100%"
          transform="translate(42.576 12)"
          xlinkHref="#youAreHereIcon"
        />
      </g>
    </svg>
  );
}
